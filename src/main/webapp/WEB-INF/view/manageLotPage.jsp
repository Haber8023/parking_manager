...
<body>
												...
												<tr>
													...
													<td>
														<button id="update" type="button"
															...
															class="btn btn-warning btn-sm" data-toggle="modal">修改停车场信息</button>
														<button id="manageSpace" type="button"
															onclick="spaceModal(this)" class="btn btn-success btn-sm"
															data-toggle="modal">管理车位</button>
														<button id="insertSpace" type="button"
															onclick="insertSpaceModal(this)"
															class="btn btn-primary btn-sm" data-toggle="modal">新增车位</button>
													</td>
												</tr>
											</c:forEach>
										</table>
	...
	<div class="modal fade" id="updateModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<form name="updatelotForm" id="updatelotForm" 
				action="updateLot" onsubmit="return validateForm()" 
				method="post"
					class="form-validate">
					...
					<div class="modal-body">
						停车场名： <input id="lot_name" type="text" name="lot_name">
					</div>
					<div class="modal-body">
	...
	<div class="modal fade" id="spaceModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="myModalLabel">管理车位信息</h4>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<form name="lotForm" id="lotForm" action="manageSpacePage"
					method="post" class="form-validate">
					<div class="modal-body">
						停车场ID： <input style="border: none;" id="lot_id_manage" type="text" name="lot_id_manage"
							readonly>
					</div>
					<div class="modal-body">
						停车场名： <input style="border: none;" id="lot_name_manage" type="text"
							name="lot_name_manage" readonly>
					</div>
					<div class="modal-footer">
						<button type="submit" class="btn btn-success">前往管理车位信息页面</button>
					</div>
				</form>
			</div>
		</div>
	</div>
	<div class="modal fade" id="insertSpaceModal" tabindex="-1"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<form name="spaceForm" id="spaceForm" action="insertSpace"
					method="post" class="form-validate">
					<div class="modal-body">
					...
					<div class="modal-body">
						<input name="lot_id_insert" id="lot_id_insert" type="hidden"
							readonly>
					</div>
					<div class="modal-footer">
						<button type="submit" class="btn btn-success">新增车位</button>
					</div>
				</form>
			</div>
		</div>
	...
	<script>
		function spaceModal(obj) {
			$("#spaceModal").modal('show');
			var $td = $(obj).parents('tr').children('td');
			var lot_id = $td.eq(0).text();
			var lot_name = $td.eq(1).text();
			$("#lot_id_manage").val(lot_id);
			$("#lot_name_manage").val(lot_name);
		}
	</script>
	<script>
		function insertSpaceModal(obj) {
			$("#insertSpaceModal").modal('show');
			var $td = $(obj).parents('tr').children('td');
			var lot_id_insert = $td.eq(0).text();
			$("#lot_id_insert").val(lot_id_insert);
		}
	</script>
	<script>
		function validateForm() {
			var lot_name = document.forms["updatelotForm"]["lot_name"].value;
			var lot_price = document.forms["updatelotForm"]["lot_price"].value;
			var lot_free_start = document.forms["updatelotForm"]["lot_free_start"].value;
			var lot_free_end = document.forms["updatelotForm"]["lot_free_end"].value;
			var lot_status = document.forms["updatelotForm"]["lot_status"].value;
			...
			if (!myreg.test(lot_free_start) || !myreg.test(lot_free_end)) {
				alert("免费时段正确格式：HH:MM:SS(22:00:00)");
				return false;
			}
			
			var myreg = /^([2][0-3])\:([0-5][0-9]):([0-5][0-9])$/;
			if (!myreg.test(lot_free_start) ) {
				alert("免费时段开始时间需设置在20:00:00-23:59:59");
				return false;
			}
			
			var myreg = /^([0][0-9])\:([0-5][0-9]):([0-5][0-9])$/;
			if (!myreg.test(lot_free_end)) {
				alert("免费时段结束时间需设置在00:00:00-09:59:59");
				return false;
			}
